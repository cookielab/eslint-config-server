		...
		for (const [ruleName, rule] of Object.entries(resolvedRules)) {
			describe(`rule "${ruleName}"`, () => {
				...
				if (
					...
					&& ruleName !== 'first' // do not check exhaustiveness for rule with options that cannot be exhaustive
				) {
					...
					it.skip(`rule "import/${ruleName}" has configuration but does not have schema => check the documentation and the code of the rule and update configuration or send a pull request adding the schema to the rule`, () => {});
				}
			});
		}
	});
